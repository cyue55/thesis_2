...
def mfbtd(
    ...
    """Modulation filter coefficients.

    Ported from Mark's `auditoryutil`_ repo.


    .. _auditoryutil: https://gitlab.gbar.dtu.dk/marksa/auditoryutil
    ...
    b, a = butter(order=1, fc=150, btype="low", analog=False, output="ba", fs=fs)
...
class GammatoneFilterbank(nn.Module):
    ...
    def inverse(self, x, _return_delayed=False):
        ...
        _delayed : torch.Tensor, optional
...
@FilterbankRegistry.register("mfbtd")
class ModulationFilterbank(torch.nn.Module):
    ...
    def __init__(
        ...
        fbs = [IIRFilterbank(**ba, dtype=dtype) for ba in coeffs]
        self._fbs = nn.ModuleList(fbs)
        self.fc = fc
